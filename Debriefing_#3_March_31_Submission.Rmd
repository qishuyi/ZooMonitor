...
* We showed her plots and tables we created to answer her questions about Cattles and Squrrel Monkeys
* We also showed her plots related to Painted African Dogs 
    ...
    + Some additional plots were not part of our four goals, but created nevertheless for our client
* This part of the meeting was mainly about talking through some of the plots with the client

\

#### **Program** 
* We then asked her about her expectations for our final product, a program that will address all animals
* Even though we had asked similar things before, when we only had shown her some general plots back then 
* Since we had created a wider variety of plots at this point, we were curious if she had different ideas in mind. 
...
* For the program, we suggested that we create various inputs in the program so that the user can choose a factor by which they facet the data, e.g., day of week, time of day, animal's name, etc.
...
* Based on our client's requests, we finalized the plots and choices of inputs we put in the prototype program:
  ...
  + Barplots of user selected Behaviors (in percentages) per each individual animal, and accompanying tables
  + Barplots of user selected Categories (in percentages) per each individual animal, and accompanying tables
...
*Category is a variable that groups similar behaviors*
...
* It is most likely that the zoo keepers normally feed the animals around 11am since the "Exploratory & Feeding" bar is the highest around that time
* 12pm and 4pm show significantly few observations, analogous to the Painted African Dogs' case
...
ggplot(summary_Pistacio, aes(x="", y=percent, fill=fct_reorder(Activity, desc(percent)))) + geom_bar(stat="identity", width=1) +
  facet_grid(.~ Period) +
  coord_polar("y", start=0) + 
  labs(x = NULL, y = NULL, fill = NULL, title = "Pistacio's Passing and Activity", subtitle = "Raw Counts: Before = 760, After = 1760") +
  guides(fill = guide_legend(reverse = TRUE, override.aes = list(size = 1))) +
  theme_classic() + theme(axis.line = element_blank(),
                          axis.text = element_blank(),
                          axis.ticks = element_blank(),
...
* The raw counts of observations is included in the visual since they normally differ significantly between before/after
* Only Damian has the period "One month after" since there were observations soon after his death. (Which was not the case for the other events)
...
* Note that the subject animal (the one that passed away/joined the zoo) is **included** in the pie charts

##### **Excluding the Subject Animal**

```{r Visual for Without Damian}
##Damian's death
...
summary_Damian <- rbind(summary_D_before, summary_D_amonth, summary_D_after)
summary_Damian$Period <- factor(summary_Damian$Period, levels = c("Before", "One month after", "After"))
  
ggplot(summary_Damian, aes(x="", y=percent, fill=fct_reorder(Activity, desc(percent)))) + geom_bar(stat="identity", width=1) +
  facet_grid(.~ Period) +
  coord_polar("y", start=0) + 
  labs(x = NULL, y = NULL, fill = NULL, title = "Damian's Passing and Activity (without Damian)", subtitle = "Raw Counts: Before = 317, One month after = 158, After = 1841") +
  guides(fill = guide_legend(reverse = TRUE, override.aes = list(size = 1))) +
  theme_classic() + theme(axis.line = element_blank(),
                          axis.text = element_blank(),
                          axis.ticks = element_blank(),
...
summary_Pistacio <- rbind(summary_P_before, summary_P_after)
summary_Pistacio$Period <- factor(summary_Pistacio$Period, levels = c("Before", "After"))

ggplot(summary_Pistacio, aes(x="", y=percent, fill=fct_reorder(Activity, desc(percent)))) + geom_bar(stat="identity", width=1) +
  facet_grid(.~ Period) +
  coord_polar("y", start=0) + 
  labs(x = NULL, y = NULL, fill = NULL, title = "Pistacio's Passing and Activity (without Pistacio)", subtitle = "Raw Counts: Before = 591, After = 1673") +
  guides(fill = guide_legend(reverse = TRUE, override.aes = list(size = 1))) +
  theme_classic() + theme(axis.line = element_blank(),
                          axis.text = element_blank(),
                          axis.ticks = element_blank(),
...
summary_Squirt <- rbind(summary_S_before, summary_S_after)
summary_Squirt$Period <- factor(summary_Squirt$Period, levels = c("Before", "After"))

ggplot(summary_Squirt, aes(x="", y=percent, fill=fct_reorder(Activity, desc(percent)))) + geom_bar(stat="identity", width=1) +
  facet_grid(.~ Period) +
  coord_polar("y", start=0) + 
  labs(x = NULL, y = NULL, fill = NULL, title = "Squirt's Joining and Activity (without Squirt)", subtitle = "Raw Counts: Before = 1296, After = 1016") +
  guides(fill = guide_legend(reverse = TRUE, override.aes = list(size = 1))) +
  theme_classic() + theme(axis.line = element_blank(),
                          axis.text = element_blank(),
                          axis.ticks = element_blank(),
...
* Note that the subject animal (the one that passed away/joined the zoo) is **excluded** in the pie charts, that is to say, both pie charts (before and after) represent the proportion of the behaviors of all animals but the subject one.
* These pie charts, with the exclusion of the subject animals, convey the impact of their death/joining on the other animals. 
...
* We have provided three datasets that the user can choose so that we do not have to send Professor Miller and STA-395 class the CSV Files of the animals' data.
...
* Creation of the generalized cleaning script
  ...
  + This script takes the into account the differences among the datasets and cleans any dataset in the same way 
* Meeting with our client 
  + She told us her more specific preferences on the plots to include in the program
...
* Pie chart's inclusion/exclusion of the subject animal
  ...
  + The problem we encountered is that the subject individual will be present in only one of the pies (before or after), and this might affect the behavior proportion
  ...
  + Ended up including the subject animal because we wanted to broaden the functionality of the app by not limiting the events to some animal's death/joining but allowing it to include the events that do not involve any animal, such as earthquake and renovation of the zoo. 
* Pie chart: fasely reported events' dates
  + Anna reported that Damian and Pistacio passed away on 2/22/2018 and 12/3/2018, repectively.
  + However, the observations of them existed after those dates, even until recently.
  + Therefore, by excluding the subject animal (Damian or Pistacio), we obtained a decreased row count for the "After" preiord, which is not supposed to be the case (since they are not supposed to have existed then)
  + Since Anna did not know the actual dates of their deaths, this issue is yet to be solved. 
  + This does not affect our app because we let the users choose any date, so as long as they know the actual date of the event, the app must function properly.
